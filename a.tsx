...
import { 
  ...
  Shield, User, UserCog, CheckSquare, Square, Info, Archive, Database
} from 'lucide-react';
...
export default function App() {
  ...
  const handleLogin = (e) => {
    ...
    if (found) {
        ...
        else if (found.permissions.includes('tab_team')) { setActiveTab('team'); setActiveTeamTab(found.permissions.includes('sub_finance') ? 'finance_team' : found.permissions.includes('sub_overview') ? 'task_overview' : 'task_management'); }
                  ...
                  <div className="space-y-4">
                      ...
                      {editingAppUser.role === 'user' && (
                          <div className="pt-2 border-t border-slate-100">
                                              ...
                                              <label key={perm.id} className={`flex items-center gap-3 cursor-pointer ${perm.isSub ? 'ml-6' : ''} ${!parentChecked && perm.isSub ? 'opacity-40 pointer-events-none' : ''}`}>
                                                  <div onClick={() => {
                                                      ...
                                                      if (isChecked) {
                                                          ...
                                                          if (!perm.isSub && perm.id === 'tab_team') newPerms = newPerms.filter(p => !['sub_finance', 'sub_overview', 'sub_task'].includes(p));
                                                          ...
                                                          if (!perm.isSub && perm.id === 'tab_team') newPerms.push('sub_finance', 'sub_overview', 'sub_task');
                                                      }
                              ...
                              {/* QUYỀN XEM NHÓM */}
                              {editingAppUser.permissions.includes('sub_finance') && (
                                  <div className="mt-3 pt-3 border-t border-slate-100 animate-in slide-in-from-top-2">
                                      <label className="block text-sm font-bold text-slate-800 mb-2">Quyền xem nhóm (Tài chính)</label>
